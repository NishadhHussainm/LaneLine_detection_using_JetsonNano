...
def draw_lines(img, lines, color=(18, 23, 124), thickness=5):
    if lines is None:
        return
    for line in lines:
        if line is not None:
...
def lane_detection(image):
    ...
    if lines is not None:
        ...
        # Draw the shaded region between the lines
        if left_line_points is not None and right_line_points is not None:
            pts = np.array([left_line_points[0][:2], left_line_points[0][2:], 
                            right_line_points[0][2:], right_line_points[0][:2]], np.int32)
            pts = pts.reshape((-1, 1, 2))
            cv2.fillPoly(image, [pts], color=(124, 18, 26)) 
    
    return image

cap = cv2.VideoCapture('/dev/video0')  # Use 0 for built-in webcam or change for external camera
